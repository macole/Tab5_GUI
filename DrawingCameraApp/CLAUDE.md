...
```bash
# Prerequisites: ESP-IDF v5.4.1 or v5.5.1 installed
# Step 1: Fetch dependencies (first time or after dependency updates)
python3 ./fetch_repos.py

# Step 2: Activate ESP-IDF environment
source ~/esp/v5.5.1/esp-idf/export.sh
# Or for v5.4.1: source ~/esp/esp-idf/export.sh
...
# Step 5: Flash to device (connect M5Tab5 via USB first)
idf.py flash
...
**注意事項**:
- 初回ビルド前、または依存関係が更新された場合は必ず `python3 ./fetch_repos.py` を実行してください
- ESP-IDF環境は各ターミナルセッションで `export.sh` を実行してアクティブ化する必要があります
- 転送前にM5Tab5がUSBケーブルで正しく接続されていることを確認してください
