...
#include <stdlib.h>  // for free() function when LV_MEM_CUSTOM == 1
...
void _ui_screen_change(lv_obj_t ** target, lv_scr_load_anim_t fademode, int spd, int delay, void (*target_init)(void))
{
    if(*target == NULL)
        target_init();
    // LVGL 9.xではlv_screen_load_anim()を使用
...
void _ui_arc_increment(lv_obj_t * target, int val)
{
    ...
    // LVGL 9.xではlv_obj_send_event()を使用
...
void _ui_state_modify(lv_obj_t * target, int32_t state, int value)
{
    if(value == _UI_MODIFY_STATE_TOGGLE) {
        if(lv_obj_has_state(target, state)) lv_obj_remove_state(target, state);  // LVGL 9.xではremove_state
        else lv_obj_add_state(target, state);
    }
    else if(value == _UI_MODIFY_STATE_ADD) lv_obj_add_state(target, state);
    else lv_obj_remove_state(target, state);  // LVGL 9.xではremove_state
}
...
{
    // LVGL 9.xではlv_free()を使用（LV_MEM_CUSTOMが1の場合はfree()を使用）
    #if LV_MEM_CUSTOM == 1
        free(a->user_data);
    #else
        lv_free(a->user_data);
