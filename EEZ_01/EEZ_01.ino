...
// LVGL設定（Canvas描画を滑らかにするため短めに設定）
#define LVGL_TIMER_DELAY_MS 5   // LVGLタイマー遅延（ミリ秒）

// アプリケーション設定（カスタマイズ可能）
#define APP_UPDATE_INTERVAL_MS 10  // アプリケーション更新間隔

// canvas1 描画用（screens.c の canvas1 サイズに合わせる）
#define CANVAS1_WIDTH  991
#define CANVAS1_HEIGHT 676
#define CANVAS1_BUF_SIZE ((size_t)CANVAS1_WIDTH * CANVAS1_HEIGHT * 2)  // RGB565
...
// canvas1 用バッファ（SPIRAMに確保）
static uint8_t *g_canvas1_buf = nullptr;
...
// canvas1 指描き機能
// ============================================================================
/** canvas1 タッチ描画用：前回の座標と線スタイル */
static int16_t s_canvas_last_x = -1;
static int16_t s_canvas_last_y = -1;
static bool s_canvas_drawing = false;

/** 線の太さ・色（必要に応じて変更可能） */
#define CANVAS_LINE_WIDTH  4
#define CANVAS_LINE_COLOR  lv_color_hex(0x000000)

/**
 * @brief canvas1 のタッチ描画イベントコールバック
 * PRESSED/PRESSING で線を描画、RELEASED でストローク終了。
 */
static void canvas_draw_event_cb(lv_event_t *e)
{
    ...
    int16_t x = point.x - area.x1;
    int16_t y = point.y - area.y1;

    /* キャンバス内にクランプ */
    if (x < 0) x = 0;
    if (x >= CANVAS1_WIDTH)  x = CANVAS1_WIDTH - 1;
    if (y < 0) y = 0;
    if (y >= CANVAS1_HEIGHT) y = CANVAS1_HEIGHT - 1;

    if (code == LV_EVENT_PRESSED) {
        s_canvas_last_x = x;
        s_canvas_last_y = y;
        s_canvas_drawing = true;
        lv_canvas_set_px_color(canvas, (lv_coord_t)x, (lv_coord_t)y, CANVAS_LINE_COLOR);
    } else if (code == LV_EVENT_PRESSING && s_canvas_drawing) {
        if (s_canvas_last_x >= 0 && s_canvas_last_y >= 0) {
            lv_point_t points[2] = { { (lv_coord_t)s_canvas_last_x, (lv_coord_t)s_canvas_last_y }, { (lv_coord_t)x, (lv_coord_t)y } };
            lv_draw_line_dsc_t line_dsc;
            lv_draw_line_dsc_init(&line_dsc);
            line_dsc.color = CANVAS_LINE_COLOR;
            line_dsc.width = CANVAS_LINE_WIDTH;
        ...
        s_canvas_drawing = false;
        s_canvas_last_x = -1;
        s_canvas_last_y = -1;
    }
}

static void setupCanvasDrawing()
{
    if (!objects.canvas1) return;

    /* Canvas は Image を継承しており、デフォルトでクリック無効のためPRESSED/PRESSING/RELEASED が届かない。CLICKABLE を有効にする。 */
    lv_obj_add_flag(objects.canvas1, LV_OBJ_FLAG_CLICKABLE);

    g_canvas1_buf = (uint8_t *)heap_caps_malloc(CANVAS1_BUF_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!g_canvas1_buf) {
        return;  /* SPIRAM 不足時はスキップ */
    }

    lv_canvas_set_buffer(objects.canvas1, g_canvas1_buf, CANVAS1_WIDTH, CANVAS1_HEIGHT, LV_IMG_CF_TRUE_COLOR);
    lv_canvas_fill_bg(objects.canvas1, lv_color_white(), LV_OPA_COVER);

    lv_obj_add_event_cb(objects.canvas1, canvas_draw_event_cb, LV_EVENT_PRESSED, NULL);
    lv_obj_add_event_cb(objects.canvas1, canvas_draw_event_cb, LV_EVENT_PRESSING, NULL);
    lv_obj_add_event_cb(objects.canvas1, canvas_draw_event_cb, LV_EVENT_RELEASED, NULL);
}

/**
 * @brief EEZ Studio アクション：Canvas1 を白でクリア（Button1 から呼ばれる）
 * actions.h で宣言され、screens.c の Button1 イベントで参照される。
 * C からリンクするため extern "C" が必須。
 */
#ifdef __cplusplus
extern "C" {
#endif
void action_clear_canvas1(lv_event_t *e)
{
    (void)e;
    if (objects.canvas1 && g_canvas1_buf) {
        lv_canvas_fill_bg(objects.canvas1, lv_color_white(), LV_OPA_COVER);
    }
}
#ifdef __cplusplus
