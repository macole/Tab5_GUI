# トラブルシューティングガイド - LV8wF_Clock

このドキュメントは、LV8wF_Clockプロジェクトで発生する可能性のある問題とその解決策をまとめています。

## 📑 目次

### 基本問題
1. [画面がチカチカして動かない](#-画面がチカチカして動かない)
2. [EEZ Flowのアサーションエラー（リブートループ）](#6-eez-flowのアサーションエラーリブートループ)
3. [タッチパネルの反応が悪い](#7-タッチパネルの反応が悪いボタンを何度も押さないと反応しない)

### WiFi/NTP関連
4. [WiFi接続に失敗する](#8-wifi接続に失敗する)
5. [NTP時刻同期に失敗する](#9-ntp時刻同期に失敗する)
6. [text_areaに時刻が表示されない](#10-text_areaに時刻が表示されない)
7. [文字が□として表示される](#11-文字がとして表示される)
8. [時刻が更新されない](#12-時刻が更新されない)

### デバッグ・設定
- [デバッグ手順](#-デバッグ手順)
- [ボード設定チェックリスト](#️-ボード設定チェックリスト)
- [応急処置: 最小構成で動作確認](#-応急処置-最小構成で動作確認)
