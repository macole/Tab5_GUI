...
static int g_lastBrightness = -1;  // 前回の明るさ値（変更検出用）
 ...
 * @brief 画面の明るさを更新
 * 
 * Flow変数 "light" の値（0-255）を取得して、画面の明るさを変更します。
 * 値が変更された場合のみ更新します。
 */
void updateBrightness()
{
    ...
    // Flow変数 "light" の値を取得
    Value lightValue = getVar(FLOW_GLOBAL_VARIABLE_LIGHT);
    
    // 整数値に変換（0-255の範囲を想定）
    int brightness = lightValue.toInt32();
    ...
    // 値が変更された場合のみ更新
    if (brightness != g_lastBrightness) {
